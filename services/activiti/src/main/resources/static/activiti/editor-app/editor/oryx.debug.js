...
ORYX.CONFIG.BPMN20_SCHEMA_VALIDATION_ON = true;
	...
	/* Namespaces */
ORYX.nacos.namespace_ORYX =			"http://www.b3mn.org/oryx";
ORYX.nacos.namespace_SVG =				"http://www.w3.org/2000/svg";
...
ORYX.Core.SVG.Label = Clazz.extend({
	...
	construct: function(options) {
		...
		this.fitToElemId = this.node.getAttributeNS(ORYX.nacos.namespace_ORYX, 'fittoelem');
		if(this.fitToElemId)
			this.fitToElemId = this.shapeId + this.fitToElemId;

		//set alignment
		var alignValues = this.node.getAttributeNS(ORYX.nacos.namespace_ORYX, 'align');
		if(alignValues) {
		...
		this.edgePosition = this.node.getAttributeNS(ORYX.nacos.namespace_ORYX, 'edgePosition');
		...
		//get offset top
		this.offsetTop = this.node.getAttributeNS(ORYX.nacos.namespace_ORYX, 'offsetTop') || ORYX.CONFIG.OFFSET_EDGE_LABEL_TOP;
		if(this.offsetTop) {
			this.offsetTop = parseInt(this.offsetTop);
		}

		//get offset top
		this.offsetBottom = this.node.getAttributeNS(ORYX.nacos.namespace_ORYX, 'offsetBottom') || ORYX.CONFIG.OFFSET_EDGE_LABEL_BOTTOM;
		...
		//set rotation
		var rotateValue = this.node.getAttributeNS(ORYX.nacos.namespace_ORYX, 'rotate');
		if(rotateValue) {
			try {
		...
		//anchors
		var anchorAttr = this.node.getAttributeNS(ORYX.nacos.namespace_ORYX, "anchors");
		if(anchorAttr) {
			anchorAttr = anchorAttr.replace("/,/g", " ");
			var anchors = anchorAttr.split(" ").without("");
	...
	update: function(force) {
		...
		if(this._isChanged || x !== this.oldX || y !== this.oldY || force === true) {
			if (this.isVisible) {
				...
				//this.node.setAttributeNS(ORYX.nacos.namespace_ORYX, 'align', this._horizontalAlign + " " + this._verticalAlign);
				...
				if (this.node.ownerDocument) {
					...
					if (this.fitToElemId || this._textHasChanged){
						...
						textLines.each((function(textLine, index){
							var tspan = this.node.ownerDocument.createElementNS(ORYX.nacos.namespace_SVG, 'tspan');
	...
	_checkFittingToReferencedElem: function() {
		try {
			var tspans = $A(this.node.getElementsByTagNameNS(ORYX.nacos.namespace_SVG, 'tspan'));
						...
						if (textLength > refBoxLength) {
							...
							for (var i = 0; i < numOfChars; i++) {
								...
								if (sslength > refBoxLength - 2) {
									var newtspan = this.node.ownerDocument.createElementNS(ORYX.nacos.namespace_SVG, 'tspan');
...
//				}
	...
	getFontSize: function(node) {
		var tspans = this.node.getElementsByTagNameNS(ORYX.nacos.namespace_SVG, 'tspan');
...
*/
	...
	getSVGRepresentation: function(escapeText) {
		...
		if(escapeText) {
			$A(svgClone.getElementsByTagNameNS(ORYX.nacos.namespace_SVG, 'tspan')).each(function(elem) {
				elem.textContent = elem.textContent.escapeHTML();
			});

			$A(svgClone.getElementsByTagNameNS(ORYX.nacos.namespace_SVG, 'text')).each(function(elem) {
		...
		$A(svgClone.getElementsByTagNameNS(ORYX.nacos.namespace_SVG, 'image')).each(function(elem) {
		...
		// escape all links
		$A(svgClone.getElementsByTagNameNS(ORYX.nacos.namespace_SVG, 'a')).each(function(elem) {
												...
												if (svgElem.tagName.toLowerCase() === "stop"){
													...
													if (svgElem.parentNode.tagName.toLowerCase() === "radialgradient"){
														$A(svgElem.parentNode.getElementsByTagName('stop')).each(function(stop){
															stop.setAttributeNS(null, "stop-opacity", prop ? stop.getAttributeNS(ORYX.nacos.namespace_ORYX, 'default-stop-opacity') || 1 : 0);
														}.bind(this))
													}
												} else {
...
ORYX.Core.Node = {
    ...
    _init: function(svgDocument){
        ...
        var minSizeAttr = svgNode.getAttributeNS(ORYX.nacos.namespace_ORYX, "minimumSize");
        if (minSizeAttr) {
        ...
        var maxSizeAttr = svgNode.getAttributeNS(ORYX.nacos.namespace_ORYX, "maximumSize");
        if (maxSizeAttr) {
        ...
        var magnets = svgDocument.getElementsByTagNameNS(ORYX.nacos.namespace_ORYX, "magnets");

        if (magnets && magnets.length > 0) {

            magnets = $A(magnets[0].getElementsByTagNameNS(ORYX.nacos.namespace_ORYX, "magnet"));
            ...
            magnets.each(function(magnetElem){
                ...
                var cx = parseFloat(magnetElem.getAttributeNS(ORYX.nacos.namespace_ORYX, "cx"));
                var cy = parseFloat(magnetElem.getAttributeNS(ORYX.nacos.namespace_ORYX, "cy"));
                ...
                //get anchors
                var anchors = magnetElem.getAttributeNS(ORYX.nacos.namespace_ORYX, "anchors");
                if (anchors) {
                ...
                if (!this._defaultMagnet) {
                    var defaultAttr = magnetElem.getAttributeNS(ORYX.nacos.namespace_ORYX, "default");
        ...
        /**initialize docker */
        var dockerElem = svgDocument.getElementsByTagNameNS(ORYX.nacos.namespace_ORYX, "docker");

        if (dockerElem && dockerElem.length > 0) {
            dockerElem = dockerElem[0];
            var docker = this.createDocker();
            var cx = parseFloat(dockerElem.getAttributeNS(ORYX.nacos.namespace_ORYX, "cx"));
            var cy = parseFloat(dockerElem.getAttributeNS(ORYX.nacos.namespace_ORYX, "cy"));
            docker.bounds.centerMoveTo({
                x: cx - offsetX,
                y: cy - offsetY
            });
            ...
            //get anchors
            var anchors = dockerElem.getAttributeNS(ORYX.nacos.namespace_ORYX, "anchors");
            if (anchors) {
                anchors = anchors.replace("/,/g", " ");
                anchors = anchors.split(" ").without("");
        ...
        /**initialize labels*/
        var textElems = svgNode.getElementsByTagNameNS(ORYX.nacos.namespace_SVG, 'text');
...
ORYX.Core.Edge = {
	...
	*/
    ...
    _init: function(svgDocument){
		...
		/**initialize labels*/
        var textElems = svgDocument.getElementsByTagNameNS(ORYX.nacos.namespace_SVG, 'text');
